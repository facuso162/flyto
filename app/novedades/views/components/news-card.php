...
$nextNewsButtonMode = $nextNewsButtonMode ?? 'link';
...
<article class="border border-flyto-ink/10 bg-white p-6 md:border-0 md:bg-transparent md:p-0">
    <div class="flex flex-wrap items-start justify-between gap-3">
        <p class="font-mono text-xs uppercase tracking-[1.2px] text-flyto-muted">
            <?= htmlspecialchars($news['fecha'] ?? '', ENT_QUOTES, 'UTF-8') ?>
        </p>
    ...
    <p class="mt-4 text-sm leading-[22.75px] text-flyto-muted">
        <?= htmlspecialchars($news['resumen'] ?? '', ENT_QUOTES, 'UTF-8') ?>
    </p>
    <?php if ($showNextNewsButton): ?>
        <?php if ($nextNewsButtonMode === 'button'): ?>
        ...
        <?php else: ?>
            <a href="<?= htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8') ?>/novedades" class="mt-6 inline-flex items-center gap-2 text-xs font-medium text-flyto-muted hover:text-flyto-navy">
                Siguiente noticia
                <span aria-hidden="true">&rarr;</span>
            </a>
        <?php endif; ?>
