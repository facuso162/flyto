...
class EditarAerolineaActionController
{
    ...
    public function editar(array $params = [], array $query = []): void
    {
        try {
            ...
            Flash::error('Necesitas permisos de administrador para editar una aerolinea.');
            RedirectResponse::to($exception->getStatusCode() === 401 ? '/auth/login' : '/', [], 303);
            return;
        }

        $aerolineaId = $this->aerolineaId($_POST);
        $editPath = $aerolineaId > 0 ? '/admin/aerolineas/editar?id=' . $aerolineaId : '/admin/aerolineas';

        try {
            CrearAerolineaValidator::validate($_POST);
            $this->aerolineaService->editar($aerolineaId, $this->dtoFromPost($_POST));

            Flash::success('Aerolinea editada correctamente.');
            RedirectResponse::to('/admin/aerolineas', [], 303);
        } catch (HttpException $exception) {
            Flash::error('No pudimos editar la aerolinea. Revisa los datos e intentalo nuevamente.');
            ...
            Flash::error('No pudimos editar la aerolinea. Intentalo nuevamente en unos minutos.');
            Flash::old($this->safeOldInput($_POST));
            RedirectResponse::to($editPath, [], 303);
        }
    }
