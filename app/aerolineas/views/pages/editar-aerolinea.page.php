...
$initialInput = [
    ...
    'paisId' => (string) $aerolinea->pais->id,
    'descripcion' => $aerolinea->descripcion,
];
$formInput = array_merge($initialInput, $oldInput);
$value = static fn (string $field): string => htmlspecialchars((string) ($formInput[$field] ?? ''), ENT_QUOTES, 'UTF-8');
$selected = static fn (int $paisId): string => (string) ($formInput['paisId'] ?? '') === (string) $paisId ? ' selected' : '';
...
<section class="px-5 py-8 sm:px-8">
    <div class="mx-auto max-w-[768px]">
        <p class="font-mono text-xs uppercase tracking-[0.1em] text-flyto-muted">Edici&oacute;n de aerol&iacute;nea</p>
        <h1 class="mt-1 font-display text-3xl font-medium tracking-tight">Editar aerol&iacute;nea</h1>
        ...
        <form method="post" action="<?= htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8') ?>/admin/aerolineas/editar" class="mt-6" novalidate>
            <input type="hidden" name="id" value="<?= (int) $aerolinea->id ?>">
            ...
            <div class="mt-6 flex items-center justify-between gap-3">
                ...
                <button type="submit" class="flex h-[42px] items-center justify-center gap-2 bg-flyto-navy px-6 text-sm font-medium text-flyto-sand transition hover:bg-flyto-ink">
                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2Z" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/><path d="M7 21v-8h10v8M7 3v5h8" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/></svg>
                    Guardar cambios
                </button>
            </div>
        </form>
