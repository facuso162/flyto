                    ...
                    <?php for ($index = 0; $index < $cantidadPasajeros; $index++): ?>
                        ...
                        <fieldset class="border border-flyto-ink/10 bg-white">
                            ...
                            <div class="grid gap-x-4 gap-y-4 p-6 sm:grid-cols-2">
                                ...
                                <label><span class="<?= $labelClass ?>">Teléfono de contacto</span><input class="<?= $fieldClass ?>" type="tel" inputmode="numeric" pattern="[0-9]*" name="pasajeros[<?= $index ?>][telefonoContacto]" value="<?= $html($old['telefonoContacto'] ?? '') ?>" placeholder="1123456789" maxlength="15" required><?= $fieldError($index, 'telefonoContacto') ?></label>
                            </div>
                        </fieldset>
                    <?php endfor; ?>
